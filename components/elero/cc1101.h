...
/// True if MARCSTATE is a transient calibration or RX/TX turn-around state
/// that will naturally transition to RX without intervention.
constexpr bool marcstate_is_transient(uint8_t marc) {
  return (marc >= CC1101_MARCSTATE_VCOON_MC && marc <= CC1101_MARCSTATE_ENDCAL) ||
         ...
         marc == CC1101_MARCSTATE_TX_END || marc == CC1101_MARCSTATE_RXTX_SWITCH;
}
